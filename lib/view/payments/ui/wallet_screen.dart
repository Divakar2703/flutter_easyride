...
class WalletScreen extends StatefulWidget {
  ...
  final bool? fromCarSelectionScreen;

  const WalletScreen({super.key, this.backVisible, this.fromCarSelectionScreen});
  ...
  Widget build(BuildContext context) {
    return PopScope(
      onPopInvokedWithResult: (d, r) {
        if (widget.fromCarSelectionScreen ?? false) {
          context.read<CarSelectionProvider>().getVehicles(context.read<BookNowProvider>().markerPositions);
        }
      },
      child: Scaffold(
        body: Column(
                                ...
                                GestureDetector(
                                  onTap: () async {
                                    if (_key.currentState?.validate() ?? false) {
                                      final check =
                                          await context.read<PaymentProvider>().addMoneyToWallet(moneyCon.text);
                                      if (check) {
